...
use super::{DisplayShared, Display};
use crate::window;
...
impl Window
{
    pub(in super) fn new(shared: Rc<RefCell<DisplayShared>>) -> Window {
        Window {
...
impl window::Window<Display> for Window
{
    ...
    fn show (&mut self, _: window::State)
    {
        ...
        let surf = disp_shared.compositor.create_surface(
            |np| np.implement(|_, _| {}, ())
        ).unwrap();

        let xdg_surf = disp_shared.xdg_shell.get_xdg_surface(
