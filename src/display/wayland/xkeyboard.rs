...
pub struct XKeyboard {
    ...
    keycode_table: Vec<key::Code>,
}

pub fn keyboard_event(disp: &Rc<DispInner>, ev: wl_keyboard::Event) {
    match ev {
        wl_keyboard::Event::Keymap { format, fd, size } => {
            let context = xkb::Context::new(xkb::CONTEXT_NO_FLAGS);
            let format = match format {
                wl_keyboard::KeymapFormat::XkbV1 => xkb::KEYMAP_FORMAT_TEXT_V1,
                _ => 0,
            };
            let keymap =
                xkb::Keymap::new_from_fd(&context, fd, size as _, format, xkb::COMPILE_NO_FLAGS)
                    .expect("Could not compile keymap given by Compositor");
            let state = xkb::State::new(&keymap);
            *disp.xkb.borrow_mut() = Some(XKeyboard {
                _context: context,
                _keymap: keymap,
                state,
        ...
        wl_keyboard::Event::Enter{serial: _, surface, keys: _} => {
            ...
            *disp.kbd_window.borrow_mut() = Some(w);
        }
        wl_keyboard::Event::Leave{serial: _, surface: _} => {
            *disp.kbd_window.borrow_mut() = None;
        }
        wl_keyboard::Event::Modifiers{serial: _, mods_depressed, mods_latched, mods_locked, group} => {
            if let Some(xkb) = disp.xkb.borrow_mut().as_mut() {
                xkb.state.update_mask(mods_depressed, mods_latched, mods_locked, 0, 0, group);
            }
        }
        wl_keyboard::Event::Key{serial: _, time: _, key, state} => {
            if let (Some(xkb), Some(w)) = (disp.xkb.borrow_mut().as_mut(), disp.kbd_window.borrow().as_ref()) {
                let key = key+8;
                let code = xkb.keycode(key);
                let xsym = xkb.state.key_get_one_sym(key);
                let sym = xkb.keysym(xsym);
                let mod_mask = match code {
                    key::Code::LeftCtrl => key::Mods::LEFT_CTRL,
                    key::Code::LeftShift => key::Mods::LEFT_SHIFT,
                    key::Code::LeftAlt => key::Mods::LEFT_ALT,
                    key::Code::RightCtrl => key::Mods::RIGHT_CTRL,
                    key::Code::RightShift => key::Mods::RIGHT_SHIFT,
                    key::Code::RightAlt => key::Mods::RIGHT_ALT,
                    key::Code::LeftSuper | key::Code::RightSuper => key::Mods::SUPER,
                    _ => key::Mods::empty(),
                };
                let mut mods = disp.key_mods.get();
                let ev = match state {
                    wl_keyboard::KeyState::Pressed => {
                        mods.insert(mod_mask);
                        window::Event::KeyDown(sym, code, mods, xkb.state.key_get_utf8(key))
                    }
                    wl_keyboard::KeyState::Released => {
                        mods.remove(mod_mask);
                        window::Event::KeyUp(sym, code, mods)
                    }
                };
                disp.key_mods.set(mods);
...
impl XKeyboard {
    fn keycode(&self, xcode: xkb::Keycode) -> key::Code {
        let xcode = xcode as usize;
        if xcode>= self.keycode_table.len() {
            writeln!(&mut stderr(), "keycode 0x{:x} is out of bounds", xcode).unwrap();
            return key::Code::Unknown;
        }
        self.keycode_table[xcode]
    }

    fn keysym(&self, xsym: xkb::Keysym) -> key::Sym {
...
fn build_keycode_table() -> Vec<key::Code> {
    vec![
        // 0x00     0
        key::Code::Unknown,
        key::Code::Unknown,
