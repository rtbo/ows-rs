...
pub struct XcbKeyboard {
    pub context: xkb::Context,
    pub device_id: i32,
    pub keymap: xkb::Keymap,
    pub state: xkb::State,
    pub keysym_map: HashMap<u32, key::Sym>,
    pub keycode_table: [key::Code; 256],
    mods: u8,
}
...
impl XcbKeyboard {
    ...
    pub fn new (connection: &xcb::Connection) -> (XcbKeyboard, u8, u8) {
    ...
    //pub fn make_key_event(&mut self, xcb_ev: &xcb::KeyPressEvent) -> KeyEvent {
    //    let xcode = xcb_ev.detail() as xkb::Keycode;
    //    let xsym = self.state.key_get_one_sym(xcode);
    ...
    //        if pressed { self.state.key_get_utf8(xcode) } else { "".to_string() }
    //    )
    //}

    fn mod_active(&self, name: &str) -> bool {
        let ind = self.keymap.mod_get_index(&name);
        self.state.mod_index_is_active(ind, xkb::STATE_MODS_DEPRESSED)
    }

    pub fn get_mods(&self) -> key::Mods {
        key::Mods::new(self.mods)
    }

    pub fn update_state(&mut self, ev: &xcb::xkb::StateNotifyEvent) {
        self.state.update_mask(ev.baseMods() as xkb::ModMask,
                               ev.latchedMods() as xkb::ModMask,
                               ev.lockedMods() as xkb::ModMask,
                               ev.baseGroup() as xkb::LayoutIndex,
                               ev.latchedGroup() as xkb::LayoutIndex,
                               ev.lockedGroup() as xkb::LayoutIndex);
    }

    fn get_keycode(&self, xcode: xkb::Keycode) -> key::Code {
        let xcode = xcode as usize;
        if xcode>= self.keycode_table.len() {
            warn!(target:"clue-xcb", "keycode 0x{:x} is out of bounds", xcode);
